...
class Baseline(object):
    def __init__(self, batch_size, hidden_dim, rep_seq_length,
                 max_seq_length, word_index,base_name, learning_rate=0.0004):
        """
        Args:
            batch_size:
        ...
        # Model
        with tf.variable_scope("baseline"+base_name, reuse=None) as scope:
    ...
    def train(self, sess, X, sentence, word_index, rewards):
        ...
        for t in range(0, sentence.shape[1]):
            ...
            loss += self.train_step(sess, history_update, rewards[:, t ], b_mask)
