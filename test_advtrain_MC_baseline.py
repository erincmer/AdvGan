...
def create_networks(NUM_EMB,EMB_DIM, END_TOKEN, word_index):


    generator_s2 = Generator(NUM_EMB,
                             ...
                             END_TOKEN, "gen2")
    discriminator_s2 = DiscSentence(NUM_EMB,
                                    headerSeq2Seq.BATCH_SIZE,
                                    EMB_DIM, headerSeq2Seq.HIDDEN_DIM,
                                    headerSeq2Seq.MAX_SEQ_LENGTH,
                                    word_index,
                                    END_TOKEN, "disc2")
    baseline_s2 = Baseline(NUM_EMB,EMB_DIM,headerSeq2Seq.BATCH_SIZE,
                           headerSeq2Seq.HIDDEN_DIM,
                           headerSeq2Seq.REP_SEQ_LENGTH,
                           headerSeq2Seq.MAX_SEQ_LENGTH,
                           word_index, "base2",
    ...
    return generator_s2, discriminator_s2, baseline_s2,
...
def trainS2(gen2, disc2, base2, hist_s2, reply_s2,
            all_sentences, d_steps=5, g_steps=5, lr=0.00001):
    discAcc = []
    genRew = []
    wrongCount1 = []
    wrongCount2 = []
    ...
    for ep in range(10):
        for j in range(0, hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE):
            ...
            for _ in range(3):
                ...
                Y_d[0:2 * int(headerSeq2Seq.BATCH_SIZE / 4), :] = np.squeeze(Y_train[sentence_index, :])
                Y_d[2 * int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(sentence[sentence_index, :])
                # Y_d[2 * int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(Y_train_noise[noise_index, :])

                X_d = np.copy(X)
                X_d[0:2 * int(headerSeq2Seq.BATCH_SIZE / 4), :] = np.squeeze(np.copy(X[sentence_index, :]))
                X_d[2 * int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(np.copy(X[sentence_index, :]))
    ...
    for ep in range(100000000):
        ...
        for d in range(d_steps):
            ...
            for _ in range(2):
                ...
                Y_train_index = np.random.random_integers((headerSeq2Seq.BATCH_SIZE - 1),
                                                          size=(2*int(headerSeq2Seq.BATCH_SIZE / 4), 1))
                sentence_index = np.random.random_integers((headerSeq2Seq.BATCH_SIZE - 1),
                                                           size=(2*int(headerSeq2Seq.BATCH_SIZE / 4), 1))
                # noise_index = np.random.random_integers((Y_train_noise.shape[0] - 1),
                #                                         size=(int(headerSeq2Seq.BATCH_SIZE / 2), 1))

                Y_d[0:2*int(headerSeq2Seq.BATCH_SIZE / 4), :] = np.squeeze(Y_train[Y_train_index, :])
                Y_d[2*int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(sentence[sentence_index, :])
                # Y_d[2 * int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(Y_train_noise[noise_index, :])

                X_d = np.copy(X)
                X_d[0:2*int(headerSeq2Seq.BATCH_SIZE / 4), :] = np.squeeze(np.copy(X[Y_train_index, :]))
                X_d[2*int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(np.copy(X[sentence_index, :]))
                # X_d[2 * int(headerSeq2Seq.BATCH_SIZE / 4):, :] = np.squeeze(X_noise[noise_index, :])
        ...
        #         rewards = gen2.MC_reward(sess, X, sentence, headerSeq2Seq.MC_NUM, disc2, word_index)
        #
        #         # Train step
        #         print("Training baseline ...")
        #         b_loss = base2.train(sess, X, sentence, word_index, rewards)
        ...
        # w1, w2, w3 = testS2_onTest(gen2, hist_s2, reply_s2, test_hist_s2, test_reply_s2, test_hist_s2_OOV,
        #                            test_reply_s2_OOV)
        ...
        for ii in range(hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE):
            ...
            # toolsSeq2Seq.convert_sentence_to_text(Y_train[0, :], word_index)
            # toolsSeq2Seq.convert_sentence_to_text(sentence_test[0, :], word_index)
            # input("wait")
            # Sample noisy false sentences
        ...
        num_test_set = 2
        disc_acc = disc_acc / (num_test_set * (hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE))
        print("Before epoch: %d, acc: " % (ep), disc_acc)
        ...
        print("Before Total Reward === ", gen_rew)
        ...
        for g in range(g_steps):
            ...
            j = np.random.randint((hist_s2.shape[0]-headerSeq2Seq.BATCH_SIZE-1))


            X = hist_s2[idxTrain[j:j + headerSeq2Seq.BATCH_SIZE], :]

            sentence_index = np.random.random_integers((headerSeq2Seq.BATCH_SIZE - 1),size=(size_sim, 1))
            X_d = np.copy(X)
            bsize = headerSeq2Seq.BATCH_SIZE // size_sim
            for j_in in range(size_sim):


                X_one = np.tile(np.copy(X[sentence_index[j_in], :]), (bsize, 1))


                X_d[j_in*bsize :(j_in +1) * bsize] = np.squeeze(np.copy(X_one))
            ...
            for j_in in range(size_sim):
                ...
                b_temp = np.tile(np.mean(np.array(rewards[j_in*bsize :(j_in +1) * bsize]), axis=0), (bsize, 1))

                b[j_in * bsize:(j_in + 1) * bsize] = np.squeeze(np.copy(b_temp))



            # sentence_few = np.copy(sentence)
            #
            # for j_in in range(size_sim):
            #
            #
            #     temp_sentence = np.tile(np.copy(sentence[j_in*bsize, :]), (bsize, 1))
            #
            #
            #     sentence_few[j_in*bsize :(j_in +1) * bsize] = np.squeeze(np.copy(temp_sentence))
            #
            # sentence = sentence_few
            # print("////////////////////////////")
            # toolsSeq2Seq.convert_sentence_to_text(X[0, :], word_index)
            #
            # toolsSeq2Seq.convert_sentence_to_text(sentence[0, :], word_index)
            # print("////////////////////////////")
            # toolsSeq2Seq.convert_sentence_to_text(X[1, :], word_index)
            #
            # toolsSeq2Seq.convert_sentence_to_text(sentence[1, :], word_index)
            # print("////////////////////////////")
            # toolsSeq2Seq.convert_sentence_to_text(X[2, :], word_index)
            #
            # toolsSeq2Seq.convert_sentence_to_text(sentence[2, :], word_index)
            # print("////////////////////////////")
            #
            #
            # input("Wait")
            ...
            for r_in in range(headerSeq2Seq.BATCH_SIZE):
                for j_in in range(1,headerSeq2Seq.REP_SEQ_LENGTH):

                    if rewards[r_in,j_in]!=0:
                        b[r_in,j_in] = rewards[r_in,j_in-1]
                    else:
                        b[r_in, j_in] = 0
            ...
            #
            # toolsSeq2Seq.convert_id_to_text(X[:5], word_index)
            # #
            # # input("wait")
            # toolsSeq2Seq.convert_id_to_text(sentence[:5],word_index)
            # #
            # print(rewards[:5])
            # print(b[:5])
            # input("wait")
            ...
            #     update_step = 0
            # X_d = toolsSeq2Seq.concat_hist_reply(X, sentence, word_index)
            #
            # d_r  = disc2.get_rewards(sess,X_d)
            # toolsSeq2Seq.convert_id_to_text(X_d[0:5],word_index)
            # print(d_r[0:5])
            # input("wait")
        ...
        num_test_set = 2
        disc_acc = disc_acc / (num_test_set * (hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE))
        print("After epoch: %d, acc: " % (ep), disc_acc)
        discAcc.append(disc_acc)
        ...
        for ii in range(hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE):
            ...
            gen_rew = gen_rew + np.sum(temp_rew[:, 1])
        genRew.append(gen_rew)
        print("After Total Reward === ", gen_rew)
...
def testS2(gen2, disc2, hist_s2):
    ...
    for ii in range(hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE):
        ...
        #         #       toolsSeq2Seq.convert_id_to_text(sentence_test[cc:cc+1], word_index), " Correct == ",
...
def testS2_onTest(gen2, hist_s2, reply_s2, test_hist_s2, test_reply_s2, test_hist_s2_OOV, test_reply_s2_OOV):
    ...
    print("Correct Words for Train === ", total_correct, " Wrong Words for Train ======== ", total_wrong, " Time == ",
    ...
    print("Correct Words for Test OOV === ", total_correct, " Wrong Words for Test OOV ======== ", total_wrong,
          " Time == ",
          str(datetime.now().time()))
...
NUM_EMB = embedding_matrix.shape[0]
# Model
print(NUM_EMB)
...
(
    gen2,
    disc2,
    base2) = create_networks(NUM_EMB,EMB_DIM, END_TOKEN, word_index)
...
# Pretrain
# preTrainS2(gen2, hist_s2, reply_s2, 400,True)
...
# preTrainS2(gen2, hist_s2, reply_s2, 200,True)
# gen2.restore_model(sess, savepathG_S2_Seq2Seq)
...
for e in range(100):
   ...
   trainS2(gen2, disc2, base2, hist_s2, reply_s2,all_sen_s1)



   # trainS1(gen1,disc1,base1,hist_s1,reply_s1,10,1,0.0000001)
