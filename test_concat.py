...
for _ in range(1):
    ...
    for j in range(1):
        X_train = hist_train[idxTrain[j * headerSeq2Seq.BATCH_SIZE:(j + 1) * headerSeq2Seq.BATCH_SIZE], :]
        ...
        #print("eoh: ", word_index.get("eoh"))
        #print("eos: ", word_index.get("eos"))
        #print("\nDisplay dialogue")
        #for i in over_ind:
        #    print(i)
        #    toolsSeq2Seq.convert_sentence_to_text(X_train[i,:], word_index)
        #    print(X_train[i,:])
        
        history = np.copy(X_train)
        for i in over_ind:
            print(i)
            toolsSeq2Seq.convert_sentence_to_text(X_train[i,:], word_index)
            print(X_train[i,:])

        toolsSeq2Seq.concat_hist_reply_over(history, Y_train, word_index,
                over_lines)
        ...
        for i in over_ind:
            ...
            toolsSeq2Seq.convert_sentence_to_text(history[i,:], word_index)
