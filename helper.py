...
class MonteCarloEmbeddingHelper(TrainingHelper):
  ...
  def __init__(self, inputs,sequence_length,embedding, start_tokens, end_token,sampling_probability = 1.0,time_major=False,softmax_temperature=None, seed=None,name = None):
    ...
    Raises:
      ValueError: if `start_tokens` is not a 1D tensor or `end_token` is not a
        scalar.
    """
    with ops.name_scope(name, "MonteCarloEmbeddingWrapper",
                        [embedding, sampling_probability]):
        if callable(embedding):
          self._embedding_fn = embedding
        else:
  ...
  def next_inputs(self, time, outputs, state, sample_ids, name=None):
    ...
    (finished, base_next_inputs, state) = (
        super(MonteCarloEmbeddingHelper, self).next_inputs(
            time=time,
            outputs=outputs,
            state=state,
            sample_ids=sample_ids,
    ...
    def maybe_sample(sample_ids,input_finished,base_next_inputs):
        next_inputs = control_flow_ops.cond(
