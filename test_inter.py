...
import inter_dataset.make_dataset 
...
def create_folders():
    ...
    savepathG_S1_Seq2Seq_inter = 'GeneratorModel/S1/Seq2Seq_inter/'
    ...
    savepathG_S2_Seq2Seq_inter = 'GeneratorModel/S2/Seq2Seq_inter/'
    ...
    if not os.path.exists(savepathG_S1_Seq2Seq_inter):
        os.makedirs(savepathG_S1_Seq2Seq)
    ...
    if not os.path.exists(savepathG_S2_Seq2Seq_inter):
        os.makedirs(savepathG_S2_Seq2Seq)
...
def create_networks(EMB_DIM,END_TOKEN,word_index):
    ...
    return generator_s1,discriminator_s1,baseline_s1,\
           generator_s2,discriminator_s2,baseline_s2
    #,discriminator_s3,baseline_s3


def preTrainS1(gen1,hist_s1,reply_s1,genEp,saveS1, savedir):
    ...
    for ep in range(genEp):
        ...
        for i in range(num_test):
            ...
            print(toolsSeq2Seq.convert_sentence_to_text(X[i,:], word_index))
            print(toolsSeq2Seq.convert_sentence_to_text(Y_train[i,:], word_index))
            print(toolsSeq2Seq.convert_sentence_to_text(sentence[i,:], word_index))
        ...
        gen1.save_model(sess, savedir)


def preTrainS2(gen2, hist_s2, reply_s2 ,genEp,saveS2, savedir):
    hist_train = hist_s2
    ...
    for ep in range(genEp):
        ...
        gen2.save_model(sess, savedir)


def pretrainS1_disc(gen1,disc1,base1,hist_s1,reply_s1,lr=0.000001):
...
def trainS1(gen1,disc1,base1,hist_s1,reply_s1,d_steps = 10,g_steps = 1,lr=0.000001):
    ...
    duration = time.time() - start_time
    print("Disc acc on whole dataset= ",d_acc, " Time: ", str(datetime.now().time()), " Duration (s): ", duration)
...
def trainS2(gen2, old_gen2, disc2,base2,hist_s2,reply_s2, 
    ...
    for ep in range(3):
        for j in range(0, hist_s2.shape[0] // headerSeq2Seq.BATCH_SIZE):
            ...
            label_d = np.ones(headerSeq2Seq.BATCH_SIZE)
            label_d[int(headerSeq2Seq.BATCH_SIZE / 4):] = 0
...
def train_inter(gen1,gen2,test_hist_s1, ep):
    """ 
    ...
    for ii in range(test_hist_s1.shape[0] // headerSeq2Seq.BATCH_SIZE):
        ...
        Y = np.ones((headerSeq2Seq.BATCH_SIZE, headerSeq2Seq.REP_SEQ_LENGTH)) * word_index['eos']
        X2_gen = np.ones((headerSeq2Seq.BATCH_SIZE, headerSeq2Seq.MAX_SEQ_LENGTH)) * word_index['eoh']

        # Add strat dialog to X2_gen
        X2_gen[:,:2] = X1_gen[:,:2]

        # Store results
        complete_index = np.zeros(headerSeq2Seq.BATCH_SIZE)
        complete_when = np.zeros(headerSeq2Seq.BATCH_SIZE)
        api_call_S2 = np.ones((headerSeq2Seq.BATCH_SIZE, headerSeq2Seq.REP_SEQ_LENGTH)) * word_index['eos']
        ...
        print("\nStart history")
        for i in range(num_test):
            print(toolsSeq2Seq.convert_sentence_to_text(X1_gen[i,:], word_index))
            #print("")
        #input("wait")
        ...
        do_stop = False
        num_round = 0
        for num_round in range(max_round):
            ...
            X2_gen = toolsSeq2Seq.concat_hist_new(X2_gen, sen1_gen, word_index)
            X1_gen = toolsSeq2Seq.concat_hist_new(X1_gen, sen1_gen, word_index)

            num_test = 3 
            print("\nAgent1")
            for i in range(num_test):
                print(toolsSeq2Seq.convert_sentence_to_text(X1_gen[i,:], word_index))
                print(toolsSeq2Seq.convert_sentence_to_text(X2_gen[i,:], word_index))
                #print("")
            #input("wait")
            ...
            # Concat hist and answers
            X1_gen = toolsSeq2Seq.concat_hist_new(X1_gen, sen2_gen, word_index)
            X2_gen = toolsSeq2Seq.concat_hist_new(X2_gen, sen2_gen, word_index)
            ...
            # Check end of dialogue
            over_lines = toolsSeq2Seq.EOD(sen2_gen, word_index)
            for i in over_lines:
                if complete_index[i] == 0:
                    complete_index[i]=1
                    complete_when[i] = num_round 
                    api_call_S2[over_lines, :] = sen2_gen[over_lines,:]
            
            # If all dialogues are over
            if np.sum(complete_index) == complete_index.shape[0]:
                break
        
        # Add sentence that were not complete
        for i in range(complete_index.shape[0]):
            if complete_index[i]==0:
                complete_when[i] = len(sen1) + 1  
                api_call_S2[i,:] = sen2_gen[i,:]

        # Compute rewards
        rewards = toolsSeq2Seq.compute_rewards(test_hist_s1,
                api_call_S2,2)

        # Write dialogue to file
        history = test_hist_s1[ii * headerSeq2Seq.BATCH_SIZE:(ii + 1) * headerSeq2Seq.BATCH_SIZE, :]
        f = open("generated_dialogue_%d.txt" %ep, 'a')
        for l in range(headerSeq2Seq.BATCH_SIZE):

            to_write = "\n\nCompleted: " +  str(complete_index[l]) + "\n"
            to_write = to_write + "Expected api_call: " + toolsSeq2Seq.convert_sentence_to_text(test_hist_s1[l,:], word_index) + "\n"
            to_write = to_write + "Actual api_call: " + toolsSeq2Seq.convert_sentence_to_text(api_call_S2[l,:], word_index) + "\n"
            to_write = to_write + "Rewards: " + str(rewards[l]) + "\n"

            to_write = to_write + "Dialogue: \n"

            for i in range(len(sen1)):
                if i <= complete_when[l]:
                    to_write = (to_write + "\n" + 
        ...
        # Compute the rewards
        # TODO: Check vector pads
        #rewards = toolsSeq2Seq.compute_rewards(api_call_S2,
        #        test_hist_S1)
        #rewards = np.sum((api_call_S2 == test_hist_s1), axis=1)  
        #rewards = (rewards == np.ones(headerSeq2Seq.REP_SEQ_LENGTH)*headerSeq2Seq.REP_SEQ_LENGTH)



def testS1_onTest(gen1,gen2,test_hist_s1, ep):
    """ 
    Test s1 trained with seq2seq
    Args:
        gen1: network
...
(embedding_matrix,
train_data,test_data,
 word_index) = inter_dataset.make_dataset.create_con(True, headerSeq2Seq.MAX_SEQ_LENGTH,headerSeq2Seq.REP_SEQ_LENGTH)
...
#(gen1, gen2) = create_networks(EMB_DIM,END_TOKEN,word_index)

# adv train 
(gen1, disc1, base1, 
        gen2, disc2, base2) = create_networks(
                EMB_DIM,END_TOKEN,word_index)
...
# Pretrain
#preTrainS2(gen2, hist_s2, reply_s2, 50,True,savepathG_S2_Seq2Seq_inter)
#testS2_onTest(gen2,hist_s2,reply_s2,test_hist_s2,test_reply_s2,test_hist_s2_OOV,test_reply_s2_OOV)
#preTrainS1(gen1, hist_s1, reply_s1,50,True,savepathG_S1_Seq2Seq_inter)


# Interactive training  
gen1.restore_model(sess,savepathG_S1_Seq2Seq_inter)
gen2.restore_model(sess,savepathG_S2_Seq2Seq_inter)
train_inter(gen1,gen2,test_hist_s1, 0)
...
# Pretrain disc
#pretrainS1_disc(gen1,disc1,base1,hist_s1,reply_s1,lr=0.000001)
#
## adv train only on a1
#for ep in range(100):
#    gen_rew, disc_acc = trainS1(gen1,disc1,base1,hist_s1,reply_s1,d_steps=10,g_steps=1)
#    
#    discAcc1.append(disc_acc)
#    genRew1.append(gen_rew)
#
#    np.savetxt("./log/xp1/discAcc1.csv", np.array(discAcc1))
#    np.savetxt("./log/xp1/genRew1.csv", np.array(genRew1))
#    
#    if ep % 5 == 0:
#        start_time = time.time()
